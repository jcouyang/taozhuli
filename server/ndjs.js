...
module.exports= function(options, server) {
  ...
  that.name = options.name || "taobao";

  // Build the authentication routes required
  that.setupRoutes= function( app ) {
    app.use('/auth/taobao_callback', function(req, res){
      req.authenticate([that.name], function(error, authenticated) {
        res.writeHead(303, { 'Location': req.session.taobao_redirect_url });
        res.end('');
      });
    });
  }

  var oa = new OAuth("12643303",
                     "dd17ebb5fb7d14cea088ae71c5dfc122",
  ...
  that.authenticate= function(request, response, callback) {
    ...
    var self= this;
    if( request.getAuthDetails()['taobao_login_attempt_failed'] === true ) {
      ...
      delete request.getAuthDetails()['taobao_login_attempt_failed'];
      self.fail( callback );
    }
    else {
      if( parsedUrl.query && parsedUrl.query.code ) {
        my._oAuth.getOAuthAccessToken(parsedUrl.query.code,
                                      {redirect_uri: my._redirectUri}, function( error, access_token, refresh_token ){
                                        if( error ) callback(error)
                                          ...
                                          my._oAuth.getProtectedResource("https://taobao.com/api/v2/json/user/show", request.session["access_token"], function (error, data, response) {
                                            if( error ) {
                                              request.getAuthDetails()['taobao_login_attempt_failed'] = true;
                                              self.fail(callback);
                                            }else {
        ...
        request.session['taobao_redirect_url']= request.originalUrl;
